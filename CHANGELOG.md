...
- **`shared/lifecycle/tools/install-env.sh`**, run by `init.sh`, adds the settings that change agent
  tool calls to `"env"` in `.claude/settings.json` with their defaults. Only missing keys are
  added; a value you changed is kept. `teardown.sh` and `kit.sh` → *Manage components* remove
  them again.
- **README → *Environment variables*** documents every setting, its default and who reads it.
